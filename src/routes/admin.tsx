...
import { isHeld, isStaffAccount, useOrbitStore, usePool, useSessionUser } from "@/lib/orbit/store";
...
type ConsoleTab = "pilot" | "membres" | "contenu" | "chat" | "support";

const NAV: { id: ConsoleTab; label: string }[] = [
  { id: "pilot", label: "Pilotage" },
...
function AdminShell() {
  ...
  const [tab, setTab] = useState<ConsoleTab>("pilot");
  const onMember = pathname.startsWith("/admin/u/");
  const active = onMember ? "membres" : tab;

  useEffect(() => {
    if (!hydrated) return;
    if (!user) void navigate({ to: "/connexion" });
    else if (!user.isAdmin) void navigate({ to: "/app/feed" });
  }, [hydrated, user, navigate]);

  if (!hydrated || !user) return <SplashMark />;
  if (!user.isAdmin) return null;

  return (
    <div className="console-shell mx-auto flex min-h-dvh w-full max-w-lg flex-col">
      <header className="flex items-end justify-between gap-3 border-b border-border px-3 py-2">
        <div className="min-w-0">
          <p className="text-[10px] uppercase tracking-[0.22em] text-muted">ORBIT · console</p>
          <h1 className="truncate text-sm font-medium">{onMember ? "Fiche membre" : NAV.find((n) => n.id === active)?.label}</h1>
        </div>
        <button
      ...
      <nav
        className="fixed inset-x-0 bottom-0 z-30 mx-auto w-full max-w-lg border-t border-accent bg-bg"
        ...
        <ul className="grid grid-cols-5">
          {NAV.map((item) => (
            <li key={item.id}>
              <button
                className={cn(
                  "h-12 w-full text-[10px] uppercase tracking-wider",
                  active === item.id ? "bg-accent text-accent-fg" : "text-muted",
                )}
                onClick={() => {
                  setTab(item.id);
                  if (pathname !== "/admin") void navigate({ to: "/admin" });
                }}
              >
                {item.label}
              </button>
...
function Pilotage() {
  ...
  const members = store.users.filter((u) => !u.isNpc).length;
  const chat24 = store.messages.filter((m) => now - +new Date(m.createdAt) < 86_400_000).length;
  const unread = (store.supportMessages ?? []).filter((m) => {
    const from = store.users.find((u) => u.id === m.fromId);
    return !!from && !isStaffAccount(from) && !(m.readAt > 0);
  }).length;
  const closed = isHeld(store.chatClosedUntil, now);
  const rows: [string, string][] = [
    ["Membres", String(members)],
    ["Messages chat 24 h", String(chat24)],
    ["Tickets support non lus", String(unread)],
    ["Chat public", closed ? `Fermé${store.chatClosedUntil ? ` · ${formatHold(store.chatClosedUntil, now)}` : ""}` : "Ouvert"],
  ];
  return (
    <ul className="border border-border">
      {rows.map(([k, v]) => (
        <li key={k} className="flex items-baseline justify-between gap-3 border-b border-border px-3 py-2 text-xs last:border-b-0">
          <span className="uppercase tracking-wider text-muted">{k}</span>
...
function Contenu() {
  const [part, setPart] = useState<"feed" | "programmes" | "exercices">("feed");
  return (
    <div>
      <div className="mb-3 flex border border-border text-[10px] uppercase tracking-wider">
        {(
          [
            ["feed", "Feed"],
          ...
          <button
            key={id}
            className={cn("h-8 flex-1", part === id ? "bg-accent text-accent-fg" : "text-muted")}
            onClick={() => setPart(id)}
          >
            {label}
          </button>
      ...
      {part === "feed" ? <AdminFeed /> : part === "programmes" ? <AdminPrograms /> : <AdminExercises />}
    </div>
  );
}
...
function AdminChat() {
  ...
  const closed = isHeld(store.chatClosedUntil, now);
  const messages = [...store.messages].sort((a, b) => +new Date(b.createdAt) - +new Date(a.createdAt));
  return (
    <div>
      ...
      <p className="mt-2 text-[10px] uppercase tracking-wider text-muted">
        {closed ? "Fermé" : "Ouvert"}
        {closed && store.chatClosedUntil ? ` · ${formatHold(store.chatClosedUntil, now)}` : ""}
      </p>
      {messages.length === 0 ? (
        <p className="mt-4 text-xs text-muted">Aucun message.</p>
      ) : (
        <ul className="mt-3 border border-border">
          {messages.map((m) => {
            const author = store.users.find((u) => u.id === m.userId);
            return (
              <li key={m.id} className="border-b border-border px-2 py-1.5 text-xs last:border-b-0">
                <div className="flex items-baseline justify-between gap-2">
                  <span className="truncate">@{author?.pseudo ?? "parti"}</span>
                  <span className="shrink-0 text-[10px] text-subtle num">{formatClock(m.createdAt)}</span>
                </div>
                <p className="whitespace-pre-wrap [overflow-wrap:break-word]">{m.text}</p>
                <button className="text-[10px] uppercase tracking-wider text-danger" onClick={() => setDel(m.id)}>
                  Supprimer
                </button>
              </li>
            );
...
function AdminMembers() {
  ...
  return (
    <div>
      ...
      />
      {members.length === 0 ? (
        <p className="mt-4 text-xs text-muted">Aucun membre.</p>
      ) : (
        <ul className="mt-2 border border-border">
          {members.map((u) => {
            ...
            const muted = isHeld(store.mutedUntil[u.id]);
            return (
...
function SupportInbox() {
  ...
  return (
    <div>
      <h2 className="text-[10px] uppercase tracking-wider text-muted">Tickets ({threads.length})</h2>
      {threads.length === 0 ? (
