...
import { formatAge, formatBodyweight, formatClock, formatFr, formatHeight } from "@/lib/orbit/format";
...
type Tab = "apercu" | "compte" | "amis" | "suivi" | "support";
      ...
      <div className="mb-4 mt-3 flex gap-1 overflow-x-auto rounded-xl bg-surface-2 p-1">
        {TABS.map((t) => (
          <button
            key={t.id}
            className={cn(
              "h-10 shrink-0 rounded-lg px-3 text-xs",
...
function Compte() {
  ...
  return (
    <div>
      ...
      <h2 className="mt-8 text-sm font-medium">Mot de passe</h2>
      <div className="mt-3 grid gap-2">
        <Input type="password" placeholder="Actuel" value={curPw} onChange={(e) => setCurPw(e.target.value)} />
        <Input type="password" placeholder="Nouveau" value={newPw} onChange={(e) => setNewPw(e.target.value)} />
      ...
      {delStep === 2 ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-bg/70 px-6">
          <div className="glass-strong w-full max-w-sm rounded-2xl p-5">
            ...
            <div className="mt-3 grid gap-2">
              <Input placeholder="Pseudo exact" value={delPseudo} onChange={(e) => setDelPseudo(e.target.value)} autoComplete="off" />
              <Input
                type="password"
...
function Amis() {
  ...
  return (
    <div>
      ...
      {incoming.length === 0 && outgoing.length === 0 ? <p className="mt-2 text-sm text-muted">Aucune demande.</p> : null}
      {incoming.length > 0 ? (
        <ul className="mt-3 space-y-2">
          {incoming.map((r) => {
            const from = store.users.find((u) => u.id === r.fromId && !u.isNpc);
            if (!from) return null;
            const blocked = isStaffAccount(from) || isStaffAccount(user);
            return (
              <li key={r.id} className="rounded-xl bg-surface px-3 py-3">
                ...
                {blocked ? (
                  <p className="mt-2 text-xs text-muted">Les comptes admin ne peuvent pas être ajoutés en ami.</p>
                ) : (
      ...
      <h2 className="mt-8 text-sm font-medium">Amis</h2>
      <form
      ...
      >
        <Input placeholder="Rechercher un pseudo" value={pseudo} onChange={(e) => setPseudo(e.target.value)} />
...
function Support() {
  ...
  return (
    <div>
      {msgs.length === 0 ? (
        <p className="text-sm text-muted">Décris ton problème, un admin recevra ça.</p>
      ) : (
              ...
              <li key={m.id} className={cn("flex", mine ? "justify-end" : "justify-start")}>
                <div
                  className={cn(
                    ...
                    mine ? "bg-accent text-accent-fg" : "bg-surface",
                  )}
                >
                  <p className="whitespace-pre-wrap break-words">{m.text}</p>
                  <p className={cn("mt-1 text-[10px] num", mine ? "text-accent-fg/70" : "text-subtle")}>
      ...
      {isStaffAccount(me) ? (
        <p className="mt-4 text-sm text-muted">Les réponses se font dans Admin → Membres.</p>
      ) : (
        <form
          className="mt-4"
          onSubmit={(e) => {
            e.preventDefault();
            const r = store.sendSupport(text);
            if (!r.ok) {
              setErr(r.error);
              return;
            }
        ...
        >
          ...
          <p className="mt-1 text-right text-[10px] text-subtle num">{text.length}/200</p>
        </form>
      )}
    </div>
  );
